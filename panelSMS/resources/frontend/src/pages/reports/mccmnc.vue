          ...
          <v-data-table v-if="!IsLoading" :headers="headers" :items="getReportData" :pagination.sync="pagination"
            ...
            <template v-slot:item="{ item, index }">
              <tr class="text-left" @click="showEditModal(item)">
                ...
                <td>{{ item.selectable?.MccMnc }}</td>
                <td>{{ item.selectable?.Operadora }}</td>
            ...
            <v-card-text>
              <table class="table">
                <thead class="blue--text text-darken-2">
                  <tr>
                    <th style="width: 25%;">Destination Country</th>
                    <th style="width: 15%;">Submitted</th>
                    <th style="width: 15%;">Notified</th>
                    <th style="width: 15%;">Wrong</th>
                    <th style="width: 15%;">Forwarded</th>
                    <th style="width: 15%;">Delivered</th>
                  </tr>
                </thead>
                <tbody style="color: black;">
                  <tr>
                    <td style="width: 25%; text-align: center;">{{ this.editItem.state }}</td>
                    <td style="width: 15%; text-align: center;">{{ this.editItem.Enviados }}</td>
                    <td style="width: 15%; text-align: center;">{{ this.editItem.Notificados }}</td>
                    <td style="width: 15%; text-align: center;">{{ this.editItem.Errores }}</td>
                    <td style="width: 15%; text-align: center;">{{ this.editItem.Reenviados }}</td>
                    <td style="width: 15%; text-align: center;">{{ this.editItem.Entregados }}</td>
                  </tr>
                </tbody>
              </table>
            </v-card-text>
...
export default {
  ...
  computed: {
    ...mapGetters(['GetReportMccMncList', 'IsLoading', 'GetReportMccMnc']),
    totalSubmited() {
    ...
    getCustomers: function () {
      return this.GetReportMccMncList?.customerList?.map(item => `${item.NOMBRE} [${item.ID_MASTER}]`).splice(0, 50);
    },
  ...
  methods: {
    ...mapActions(['fetchReportMccMncListData', 'fetchMccMncData']),
    searchResult() {
      this.fetchReportMccMncListData({
