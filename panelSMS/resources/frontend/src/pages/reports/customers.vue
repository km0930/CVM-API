  ...
  <div class="position-relative">
    <VRow class="match-height">
      <VCol cols="12" md="12" lg="12">
        <VCard class="text-center pb-4" title="Search Reports for Customers">
              ...
              <table class="table">
                <thead class="blue--text text-darken-2">
                  <tr>
                    ...
                    <th style="width: 10%;">Master Name</th>
                    ...
                    <th style="width: 10%;">Forwarded</th>
                    <th style="width: 10%;">Delivered</th>
                  </tr>
                </thead>
                <tbody style="color: black;">
                  <tr>
                    <td style="width: 10%; text-align: center;">{{ this.editItem.IDCliente }}</td>
                    <td style="width: 10%; text-align: center;">{{ this.editItem.NombreCliente }}</td>
                    <td style="width: 10%; text-align: center;">{{ this.editItem.masterName }}</td>
                    <td style="width: 15%; text-align: center;">{{ this.editItem.state }}</td>
                    <td style="width: 10%; text-align: center;">{{ this.editItem.Enviados }}</td>
                    <td style="width: 10%; text-align: center;">{{ this.editItem.Notificados }}</td>
                    <td style="width: 10%; text-align: center;">{{ this.editItem.Errores }}</td>
                    <td style="width: 10%; text-align: center;">{{ this.editItem.Reenviados }}</td>
                    <td style="width: 10%; text-align: center;">{{ this.editItem.Entregados }}</td>
                  </tr>
                </tbody>
              </table>
            </v-card-text>
...
export default {
  ...
  data() {
    return {
      ...
      isBox: false,
      headers: [
        { title: "Customer ID", align: "start", key: "IDCliente" },
        { title: "Customer Name", align: "start", key: "NombreCliente" },
        { title: "Master Name", align: "start", key: "masterName" },
      ...
      editItem: {
        ...
        Mensajes_Cliente: 0,
        Mensajes_Proveedor: 0,
        Entregados: 0, // delievered
        masterName: '', // master name,
        IDCliente: 0, // customer id,
        NombreCliente: '', // customer name
      }
    };
  },
  computed: {
    ...mapGetters(["GetReportCustomers", "IsLoading", "GetReportMccMnc"]),
    totalSubmited() {
      return this.getReportData.reduce((acc, item) => acc + item.Enviados, 0);
    },
    totalNotified() {
      return this.getReportData.reduce(
        (acc, item) => acc + item.Notificados,
        0
      );
    },
    totalWrong() {
    ...
    getReportData: function () {
      if (this.GetReportCustomers?.data) {
        return this.GetReportCustomers?.data?.map((xx) => {
          let dd = xx;
    ...
    getCountries: function () {
      return this.GetReportCustomers?.countryList?.map((item) => {
        return {
          code: item.code,
          state: item.state,
    ...
    getProviders: function () {
      return this.GetReportCustomers?.providerList
        ?.map((item) => `${item.NOMBRE}[${item.ID_PROVEEDOR}]`)
        .splice(0, 50);
    },
    getCustomers: function () {
      return this.GetReportCustomers?.customerList
        ?.map((item) => `${item.NOMBRE} [${item.ID_MASTER}]`)
        .splice(0, 50);
    },
    getTypeRoute: function () {
      const routes = this.GetReportCustomers?.typeRoute || {};
      return Object.keys(routes)?.map(
        (route) => `${routes[route]} [${route.substr(1, 1)}]`
      );
    },
    getMccMnc: function () {
      return (
        this.GetReportMccMnc?.map(
          (item) => `${item.OPERATOR} [${item.MCC}${item.MNC}]`
        ).sort() ?? []
      );
    },
  ...
  methods: {
    ...mapActions(["fetchReportCustomerData", "fetchMccMncData"]),
    searchResult() {
      this.fetchReportCustomerData({
    ...
    getNumList(array) {
      return Array.isArray(array)
        ? array.map((str) => parseInt(str.match(/\d+/)[0])).join(",")
        : array;
    },
    handleDate(value) {
    ...
    showEditModal(item) {
      ...
      this.editItem.masterName = item.masterName
    ...
    closeEditModal() {
      this.editModalShow = false
      this.editItem.masterName = ''
      this.editItem.IDCliente = ''
