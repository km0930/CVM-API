            ...
            <v-card-text>
              <table class="table">
                <thead class="blue--text text-darken-2">
                  <tr>
                    <th style="width: 20%;">Destination Country</th>
                ...
                <tbody style="color: black;">
                  <tr>
                    <td style="width: 20%; text-align: center;">{{ this.editItem.state }}</td>
                    ...
                    <td style="width: 13%; text-align: center;">{{ this.editItem.Entregados }}</td>
                  </tr>
                </tbody>
              </table>
            </v-card-text>
...
export default {
  ...
  computed: {
    ...mapGetters(['GetReportSenders', 'IsLoading', 'GetReportMccMnc']),
    totalSubmited() {
    ...
    getCustomers: function () {
      return this.GetReportSenders?.customerList?.map(item => `${item.NOMBRE} [${item.ID_MASTER}]`).splice(0, 50);
    },
  ...
  methods: {
    ...mapActions(['fetchReportSenderData', 'fetchMccMncData']),
    searchResult() {
      this.fetchReportSenderData({
