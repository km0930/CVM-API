...
export default {
  ...
  data() {
    ...
    return {
      ...
      headers: [
        ...
        { title: 'Backup Route', align: 'start', key: 'backupList' },
      ],
      countryModel: null,
      typeRouteModel: null,
      connectionData: [],
  ...
  computed: {
    ...mapGetters(['GetRoutingBackupCountriesData', 'IsLoading',]),
    getCountries: function () {
      return this.GetRoutingBackupCountriesData?.countryList?.map(item => {
        return {
          code: item.code,
          state: item.state,
    ...
    getRouteTypes: function () {
      return this.GetRoutingBackupCountriesData?.routeList?.map(item => `${item.route} [${item.id}]`).splice(0, 50);
    },
  },
  mounted() {

    this.fetchRoutingBackupCountriesData({
    }).then(() => {
      this.connectionData = this.GetRoutingBackupCountriesData?.data;
    });
  },
  methods: {
    ...mapActions(['fetchRoutingBackupCountriesData',]),
    search() {
