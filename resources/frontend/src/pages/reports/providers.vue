...
export default {
  ...
  data() {
    return {
      ...
      searchQuery: "",
      users: [],
      fields: ['ID Customer', 'Customer', 'Submited', 'Notified', 'Wrong', 'Wrong', 'Forwarded', 'Delivered', 'Ratio', 'Deposit'],
      selectedFields: ['ID_Proveedor', 'Proveedor', 'Enviados', 'Notificados', 'Errores', 'Errores', 'Reenviados', 'Entregados', 'Ratio', 'Coste'],
  ...
  computed: {
    ...mapDashboardGetters(['GetReportProviders', 'IsLoading', 'GetReportMccMnc']),
    ...mapReportsGetters(["getReportDetailProviderData", "IsLoadingDetail"]),
    totalSubmited() {
  ...
  mounted() {
    ...
    this.fetchReportDetailProviderData()
  },
  methods: {
    ...mapDashboardActions(['fetchReportProviderData', 'fetchMccMncData']),
    ...mapReportsActions(["fetchReportDetailProviderData"]),
    searchResult() {
      this.fetchReportProviderData({
    ...
    showEditModal(item) {
      ...
      if (this.editItem) {
        this.users = this.getReportDetailProviderData?.data?.filter(obj_item => {
          if (obj_item.Pais_Destino == item.Pais_Destino.toString()) {
          }
