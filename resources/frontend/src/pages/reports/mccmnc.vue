  ...
  <div class="position-relative">
    <VRow class="match-height">
      <VCol cols="12" md="12" lg="12">
        <VCard class="text-center pb-4" title="Search Reports for MccMnc">
          ...
          <v-data-table v-if="!IsLoading" :headers="headers" :items="getReportData" :pagination.sync="pagination"
            ...
            <template v-slot:item="{ item, index }">
              <tr class="text-left" @click="showEditModal(item)">
                ...
                <td>{{ item.MccMnc ? item.MccMnc : 'default' }}</td>
                <td>{{ item.Operadora ? item.Operadora : 'default'}}</td>
                <td>{{ item.Enviados }}</td>
...
export default {
  ...
  data() {
    return {
      ...
      searchQuery: "",
      users: [],
      fields: ['ID_Customer', 'Customer', 'Submited', 'Notified', 'Wrong', 'Forwarded', 'Delivered', 'Ratio', 'Deposit'],
      selectedFields: ['MccMnc', 'Operadora', 'Enviados', 'Notificados', 'Errores', 'Reenviados', 'Entregados', 'Ratio', 'Coste'],
      theaderFields: ['ID Customer', 'Customer', 'Submitted', 'Notified', 'Wrong', 'Wrong %', 'Forwarded', 'Delivered', 'Ratio', 'Deposit'],
      sortKey: "",
      sortOrder: 1,
      viewMode: "table",
      currentPage: 1,
  ...
  computed: {
    ...mapDashboardGetters(['GetReportMccMncList', 'IsLoading', 'GetReportMccMnc']),
    ...mapReportsGetters(["getReportDetailMccmncData", "IsLoadingDetail"]),
    totalSubmited() {
    ...
    getReportData: function () {
      if (this.GetReportMccMncList?.data) {
        return this.GetReportMccMncList?.data?.map(xx => {
          let dd = xx;
    ...
    getCountries: function () {
      return this.GetReportMccMncList?.countryList?.map(item => {
        return {
          code: item.code,
          state: item.state,
    ...
    getProviders: function () {
      return this.GetReportMccMncList?.providerList?.map(item => `${item.NOMBRE}[${item.ID_PROVEEDOR}]`).splice(0, 50);
    },
    getCustomers: function () {
      return this.GetReportMccMncList?.customerList?.map(item => `${item.ID_CLIENTE} [${item.EMPRESA}]`).splice(0, 50);
    },
    getTypeRoute: function () {
      const routes = this.GetReportMccMncList?.typeRoute || {};
  ...
  mounted() {
    ...
    this.fetchReportMccMncListData({
    ...
    this.fetchReportDetailMccmncData()
  },
  methods: {
    ...mapDashboardActions(['fetchReportMccMncListData', 'fetchMccMncData']),
    ...mapReportsActions(["fetchReportDetailMccmncData"]),
    searchResult() {
      this.fetchReportMccMncListData({
    ...
    showEditModal(item) {
      ...
      if (this.editItem) {
        this.users = this.getReportDetailMccmncData?.data?.filter(obj_item => {
          return obj_item.MccMnc == item.MccMnc.toString()
        })
      }
      this.editModalShow = true
    },
