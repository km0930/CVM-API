      ...
      <VCol cols="12" md="12" lg="12">
        <VCard class="text-center pb-4" title="Search Reports for Countries">
          ...
          <v-data-table v-if="!IsLoading" :headers="headers" :items="getReportData" :pagination.sync="pagination"
            :items-per-page="pagination.itemsPerPage" class="elevation-1 px-4 py-2" style="min-height: 450px">
            <template v-slot:top>
            ...
            <template v-slot:item="{ item, index }">
              ...
              <tr v-if="index === this.getReportData.length - 1">
                ...
                <td class="text-left font-weight-bold">{{ totalSubmited }}</td>
                <td class="text-left font-weight-bold">{{ totalNotified }}</td>
                <td class="text-left font-weight-bold">{{ totalWrong }}</td>
                <td class="text-left font-weight-bold">{{ totalForwarded }}</td>
                <td class="text-left font-weight-bold">{{ totalDelivered }}</td>
              </tr>
            </template>
          </v-data-table>
        </VCard>
...
export default {
  ...
  data() {
    return {
      ...
      theaderFields: ['Provider', 'Submitted', 'Notified', 'Wrong', 'Wrong %', 'Forwarded', 'Delivered', 'Ratio', 'Deposit'],
      sortKey: "",
      sortOrder: 1,
      viewMode: "table",
      currentPage: 1,
  ...
  computed: {
    ...mapDashboardGetters(["GetReportCountries", "IsLoading", "GetReportMccMnc"]),
    ...mapReportsGetters(["getReportDetailCountryData", "IsLoadingDetail"]),
    totalSubmited() {
    ...
    getProviders: function () {
      return this.GetReportCountries?.providerList
        ?.map((item) => `${item.NOMBRE}[${item.ID_PROVEEDOR}]`)
        .splice(0, 50);
    },
    getCustomers: function () {
      return this.GetReportCountries?.customerList
        ?.map((item) => `${item.EMPRESA} [${item.ID_CLIENTE}]`)
        .splice(0, 50);
    },
  ...
  mounted() {
    ...
    this.fetchReportDetailCountryData()
  },
  methods: {
    ...mapDashboardActions(["fetchReportCountryData", "fetchMccMncData"]),
    ...mapReportsActions(["fetchReportDetailCountryData"]),
    searchResult() {
      this.fetchReportCountryData({
    ...
    showEditModal(item) {
      ...
      this.editItem.Pais_Destino = item.Pais_Destino
      if (this.editItem) {
        this.users = this.getReportDetailCountryData?.data?.filter(obj_item => {
          return obj_item.Pais_Destino == item.Pais_Destino.toString()
        })
      }
