...
export default {
  ...
  data() {
    return {
      ...
      searchQuery: "",
      users: [],
      fields: ['Destination Country', 'Submited', 'Notified', 'Wrong', 'Forwarded', 'Delivered', 'Ratio', 'Deposit'],
      selectedFields: ['Pais_Destino', 'Enviados', 'Notificados', 'Errores', 'Reenviados', 'Entregados', 'Ratio', 'Coste'],
      theaderFields: ['Destination Country', 'Submitted', 'Notified', 'Wrong', 'Wrong %', 'Forwarded', 'Delivered', 'Ratio', 'Deposit'],
      sortKey: "",
      sortOrder: 1,
      viewMode: "table",
      currentPage: 1,
  ...
  computed: {
    ...mapDashboardGetters(["GetReportCustomers", "IsLoading", "GetReportMccMnc"]),
    ...mapReportsGetters(["getReportDetailCustomerData", "IsLoadingDetail"]),
    totalSubmited() {
    ...
    getCustomers: function () {
      return this.GetReportCustomers?.customerList
        ?.map((item) => `${item.ID_CLIENTE} [${item.EMPRESA}]`)
        .splice(0, 100);
    },
  ...
  mounted() {
    ...
    this.fetchReportDetailCustomerData()
  },
  methods: {
    ...mapDashboardActions(["fetchReportCustomerData", "fetchMccMncData"]),
    ...mapReportsActions(["fetchReportDetailCustomerData"]),
    searchResult() {
      this.fetchReportCustomerData({
    ...
    showEditModal(item) {
      ...
      if (this.editItem) {
        this.users = this.getReportDetailCustomerData?.data?.filter(obj_item => {
