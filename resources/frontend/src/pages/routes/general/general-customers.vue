<template>
  <VCol cols="12" md="12" lg="12">
    <VCard class="text-center py-4" title="General Customer">
      <v-divider></v-divider>
          ...
          <v-card>
            <v-card-title>
              <v-row align="center" justify="space-between">
                <v-col cols="auto">
                  <span class="headline">New Route</span>
                </v-col>
                <v-col cols="auto">
                  <v-btn icon>
              ...
              <table class="table">
                ...
                <tbody style="color: black;">
                  ...
                  <tr>
                    <td>Customer</td>
                    <td>
                      <v-autocomplete label="Customers" :items="getCustomers" v-model="newCustomerModel" item-title="state"
                        item-value="code" chips closable-chips>
                        <template v-slot:chip="{ props, item }">
                          <v-chip v-bind="props" :prepend-avatar="item.raw.flag" :text="item.raw.state"></v-chip>
                        </template>
                  ...
                  <tr>
                    <td>Type of Route</td>
                    <td>
                      <v-autocomplete v-model="newRouteModel" label="Type of Route" :items="getRouteTypes" multiple></v-autocomplete>
                    </td>
                  </tr>
                  <tr>
                    <td>Ruta</td>
                    <td class="d-flex justify-space-between align-center">
                      ...
                      <VTextField style="width: 25%" label="percent" v-model="rutaPercent" type="number" :min="0" :max="100" />
                    </td>
                  </tr>
                </tbody>
              </table>
              ...
              <table class="table">
                ...
                <tbody style="color: black;">
                  ...
                  <tr>
                    <td>Customer</td>
                    <td>
                      <v-autocomplete label="Customer" :items="getCustomers" v-model="editCustomerModel" item-title="state"
                        item-value="code" chips closable-chips>
                        <template v-slot:chip="{ props, item }">
                          <v-chip v-bind="props" :prepend-avatar="item.raw.flag" :text="item.raw.state"></v-chip>
                        </template>
                  ...
                  <tr>
                    <td>Type of Route</td>
                    <td>
                      <v-autocomplete v-model="editRouteModel" label="Type of Route" :items="getRouteTypes" multiple></v-autocomplete>
                    </td>
                  </tr>
                  <tr>
                    <td>Ruta</td>
...
export default {
  ...
  computed: {
    ...mapGetters(['GetRoutingGeneralCustomersData', 'IsLoading',]),
    getCountries: function () {
      return this.GetRoutingGeneralCustomersData?.countryList?.map(item => {
        return {
          code: item.code,
          state: item.state,
    ...
    getRouteTypes: function () {
      return this.GetRoutingGeneralCustomersData?.routeList?.map(item => `${item.route} [${item.id}]`).splice(0, 50);
    },
    getCustomers: function () {
      const customerList = this.GetRoutingGeneralCustomersData?.customerList;
  ...
  mounted() {
    ...
    this.fetchRoutingGeneralCustomerData({
    }).then(() => {
      this.connectionData = this.GetRoutingGeneralCustomersData?.data;
    });
  },
  methods: {
    ...mapActions(['fetchRoutingGeneralCustomerData',]),
    search() {
    ...
    editRow(item) {
      ...
      this.editCustomerModel = item.NOMBRE_CLIENTE
    }
  },
  watch: {
  }
