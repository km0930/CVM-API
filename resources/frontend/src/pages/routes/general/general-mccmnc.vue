<template>
  <VCol cols="12" md="12" lg="12">
    <VCard class="text-center py-4" title="General Mobile Coutry Code/Mobile Network Code(MccMnc)">
      <v-divider></v-divider>
              ...
              <table class="table">
                ...
                <tbody style="color: black;">
                  ...
                  <tr>
                    <td>MccMcc</td>
                    <td>
                      <v-autocomplete label="Customers" :items="getCustomers" v-model="newMccMccModel" item-title="state"
                        item-value="code" chips closable-chips>
                        <template v-slot:chip="{ props, item }">
                          <v-chip v-bind="props" :prepend-avatar="item.raw.flag" :text="item.raw.state"></v-chip>
                        </template>
              ...
              <table class="table">
                ...
                <tbody style="color: black;">
                  ...
                  <tr>
                    <td>MccMcc</td>
                    <td>
                      <v-autocomplete label="Customer" :items="getCustomers" v-model="editMccModel" item-title="state"
                        item-value="code" chips closable-chips>
                        <template v-slot:chip="{ props, item }">
                          <v-chip v-bind="props" :prepend-avatar="item.raw.flag" :text="item.raw.state"></v-chip>
                        </template>
...
export default {
  ...
  computed: {
    ...mapGetters(['GetRoutingGeneralMccMncData', 'IsLoading',]),
    getCountries: function () {
      return this.GetRoutingGeneralMccMncData?.countryList?.map(item => {
        return {
          code: item.code,
          state: item.state,
    ...
    getRouteTypes: function () {
      return this.GetRoutingGeneralMccMncData?.routeList?.map(item => `${item.route} [${item.id}]`).splice(0, 50);
    },
  },
  mounted() {

    this.fetchRoutingGeneralMccMncData({
    }).then(() => {
      this.connectionData = this.GetRoutingGeneralMccMncData?.data;
    });
  },
  methods: {
    ...mapActions(['fetchRoutingGeneralMccMncData',]),
    search() {
    ...
    editRow(item) {
      ...
      this.editMccModel = item.NOMBRE_CLIENTE
    }
  },
  watch: {
  }
