<template>
  <VCol cols="12" md="12" lg="12">
    <VCard class="text-center py-4" title="Sender Restricted Operators">
      <v-divider></v-divider>
              ...
              <table class="table">
                ...
                <tbody style="color: black;">
                  ...
                  <tr>
                    <td>Route</td>
                    <td>
                      <v-autocomplete v-model="newCustomerModel" label="Route" :items="getRouteTypes"
                  ...
                  <tr>
                    ...
                    <td class="d-flex justify-space-between align-center">
                      <v-text-field style="width: 70%" label="Alternative Route" v-model="ruta"></v-text-field>&nbsp; % &nbsp;
              ...
              <table class="table">
                ...
                <tbody style="color: black;">
                  ...
                  <tr>
                    <td>Route</td>
                    <td>
                      <v-autocomplete v-model="editCustomerModel" label="Route" :items="getRouteTypes"
                        multiple></v-autocomplete>
                    </td>
                  </tr>
                  <tr>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      headers: [
        ...
        { title: 'ALT Ruta', align: 'start', key: 'RUTA_ALTERNATIVA' },
      ],
      countryModel: null,
      typeRouteModel: null,
      connectionData: [],
  ...
  computed: {
    ...mapGetters(['GetRoutingRestrictedOperatorsData', 'IsLoading',]),
    getCountries: function () {
      return this.GetRoutingRestrictedOperatorsData?.countryList?.map(item => {
        return {
          code: item.code,
          state: item.state,
    ...
    getRouteTypes: function () {
      return this.GetRoutingRestrictedOperatorsData?.routeList?.map(item => `${item.route} [${item.id}]`).splice(0, 50);
    },
  },
  mounted() {

    this.fetchRoutingRestrictedOperatorsData({
    }).then(() => {
      this.connectionData = this.GetRoutingRestrictedOperatorsData?.data;
    });
  },
  methods: {
    ...mapActions(['fetchRoutingRestrictedOperatorsData',]),
    search() {
