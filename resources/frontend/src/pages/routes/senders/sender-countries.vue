<template>
  <VCol cols="12" md="12" lg="12">
    <VCard class="text-center py-4" title="Routing of Sender Countries">
      <v-divider></v-divider>
              ...
              <table class="table">
                ...
                <tbody style="color: black;">
                  ...
                  <tr>
                    <td>Sender</td>
                    <td>
                      <v-autocomplete v-model="newSenderModel" label="Type of Sender" :items="getRouteTypes" multiple></v-autocomplete>
                    </td>
                  </tr>
                  <tr>
                    <td>Ruta</td>
              ...
              <table class="table">
                ...
                <tbody style="color: black;">
                  ...
                  <tr>
                    <td>Sender</td>
                    <td>
                      <v-autocomplete v-model="editSenderModel" label="Type of Sender" :items="getRouteTypes" multiple></v-autocomplete>
                    </td>
                  </tr>
                  <tr>
                    <td>Ruta</td>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      editRouteModel: null,
      editSenderModel: null
    }
  },
  computed: {
    ...mapGetters(['GetRoutingSenderCountriesData', 'IsLoading',]),
    getCountries: function () {
      return this.GetRoutingSenderCountriesData?.countryList?.map(item => {
        return {
          code: item.code,
          state: item.state,
    ...
    getRouteTypes: function () {
      return this.GetRoutingSenderCountriesData?.routeList?.map(item => `${item.route} [${item.id}]`).splice(0, 50);
    },
  },
  mounted() {

    this.fetchRoutingSenderCountriesData({
    }).then(() => {
      this.connectionData = this.GetRoutingSenderCountriesData?.data;
    });
  },
  methods: {
    ...mapActions(['fetchRoutingSenderCountriesData',]),
    search() {
